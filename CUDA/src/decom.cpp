...
void loadQuery(string dir, std::vector<std::vector<int>>& queryStream)
{
	int r, lval, rval;
...
void get_n_query(string dir, int &line)
{
	int r, lval, rval;
	string queryFile = dir + "querystream.txt";
	FILE * queryVec = fopen(queryFile.c_str(), "r");
...
void build_lrval_index(BiGraph&g, std::vector<std::vector<lrval_index_block*>>& lrval_index_u, std::vector<std::vector<lrval_index_block*>>& lrval_index_v) {
	...
	for (int l_val = 1; l_val < lrval_index_u.size(); l_val++) {
		...
		for (int r_val = lrval_index_u[l_val].size() - 1; r_val > 0; r_val--) {
			...
			if (lrval_index_u[l_val][r_val]->nodeset.size() > 0){
				pre = lrval_index_u[l_val][r_val];
				lrval_index_u[l_val][r_val]->nsize = lrval_index_u[l_val][r_val]->nodeset.size();
			} 
		}
	}
	// build right
	...
	for (int r_val = 1; r_val < lrval_index_v.size(); r_val++) {
		...
		for (int l_val = lrval_index_v[r_val].size() - 1; l_val > 0; l_val--) {
			...
			if (lrval_index_v[r_val][l_val]->nodeset.size() > 0){
				pre = lrval_index_v[r_val][l_val];
				lrval_index_v[r_val][l_val]->nsize = lrval_index_v[r_val][l_val]->nodeset.size();
			} 
		}
	}
}
